export class ManageHotelsPage {
  ...
  displayHotels(hotels) {
      ...
      <tr>
        ...
        <td>${hotel.ChainName || 'N/A'}</td>
        <td>
          ${hotel.Street || ''}<br>
          ${hotel.City || ''}, ${hotel.State || ''} ${hotel.ZipCode || ''}
        </td>
        <td>${'★'.repeat(hotel.Rating)}</td>
        <td>${hotel.HotelEmail || 'N/A'}</td>
        <td>${hotel.HotelPhoneNo || 'N/A'}</td>
        <td>${hotel.NumberOfRooms || 0}</td>
        <td>
  ...
  async showHotelForm(hotelId = null) {
      ...
      if (hotelId) {
          try {
              hotel = await this.apiService.getHotelById(hotelId);
          } catch (error) {
              ...
              <form id="hotelForm">
                ...
                <div class="row">
                  <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="hotelChain" ${hotel ? '' : 'required'}>
    ...
    `;
      ...
      document.getElementById('saveHotelBtn').addEventListener('click', async () => {
          const formData = {
              ...
              ChainID: document.getElementById('hotelChain').value || null,
